...
const ProductCard = ({ product_id }) => {
    ...
    const sellPrice = productData?.sku[0]?.sell_price;
    const originalPrice = productData?.sku[0]?.original_price;
            ...
            {productData ? (
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-1.5 px-0.5">
                        ...
                        <Link to={`/product/${product_id}`} className="w-full text-center mt-2 py-2 bg-gray-950 hover:bg-green-500 text-white text-xs font-medium rounded transition-colors duration-300 tracking-wide">
                            Buy Now
                        </Link>
                    </div>
                </div>

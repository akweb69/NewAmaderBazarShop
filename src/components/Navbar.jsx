...
const FireDot = () => (
  <motion.span
    className="inline-flex text-green-500 ml-1"
      ...
      {badge > 0 && (
        <motion.span
          ...
          className="absolute -top-1.5 -right-1.5 w-[18px] h-[18px] rounded-full
            ...
            shadow-md shadow-green-500/20 border border-white z-20"
        >
          {badge > 9 ? '9+' : badge}
        </motion.span>
      )}
...
const SearchBar = ({ query, setQuery, onSubmit, onCamera, fileRef, className = '' }) => (
  <form onSubmit={onSubmit}
    className={`flex items-center bg-slate-50 border border-slate-200 rounded-2xl
      overflow-hidden
      focus-within:border-green-500 focus-within:bg-white
      focus-within:shadow-md focus-within:shadow-green-500/5
      transition-all duration-300 ${className}`}
  >
    <HiSearch size={17} className="ml-4 text-slate-400 flex-shrink-0" />
    <input
      type="text"
      value={query}
      onChange={e => setQuery(e.target.value)}
      placeholder="Search Product…"
      className="flex-1 px-3 py-2.5 bg-transparent text-slate-800 text-sm
        placeholder-slate-400 outline-none min-w-0"
    />
    {query && (
      <motion.button type="button" initial={{ scale: 0 }} animate={{ scale: 1 }}
        onClick={() => setQuery('')}
        className="mr-1 w-5 h-5 rounded-full bg-slate-200/60 text-slate-500 hover:text-slate-800 flex items-center justify-center transition-colors">
        <HiX size={11} />
      </motion.button>
    )}
    {/* Divider */}
    ...
    <motion.button
      ...
      className="w-9 h-9 mr-1 rounded-xl bg-slate-200/50 border border-slate-200
  ...
  return (
    <>
      ...
      <motion.nav
        ...
        className={`z-50 fixed top-0 left-0 right-0 transition-all duration-300 border-b
          ${scrolled
            ? 'bg-white/95 backdrop-blur-2xl shadow-md border-slate-100'
          ...
          <div className="flex items-center justify-between gap-3 md:gap-5 h-[64px] lg:h-[72px]">
            ...
            <Link to="/" className="flex-shrink-0">
              <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                className="relative">
                {/* Glow behind logo */}
                <div className="absolute inset-0 blur-xl bg-green-500/5 rounded-full scale-150 pointer-events-none" />
                <img
                  src={logo[0]?.logo?.logo}
                  alt="AmaderBazarShop"
                  className="relative w-[110px] sm:w-[130px] md:w-[150px] h-auto object-contain"
                />
              </motion.div>
            </Link>
            ...
            <div className="flex items-center gap-1.5 md:hidden flex-shrink-0">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="w-10 h-10 rounded-2xl bg-slate-50 border border-slate-200
                  text-slate-700 hover:bg-green-50 hover:text-green-600 hover:border-green-200
                        ...
                        <Link to={path}
                          className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold transition-all duration-200
                            ${isActive
                              ...
                              : 'text-slate-600 bg-slate-50 border border-slate-100 hover:bg-slate-100 hover:text-slate-800'
                            }`}>
                ...
                <div className="flex items-center gap-2 pt-1 pb-1">
                  ...
                  <Link to="/cod_products"
                    className="ml-auto flex items-center gap-2 px-5 py-2.5 rounded-xl
                      ...
                      shadow-md shadow-green-500/20 transition-all hover:shadow-lg hover:shadow-green-500/30">
